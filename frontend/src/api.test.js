...
import { buildImagesQuery, setDecision, fetchMeta, assignFace } from './api.js'
import { DEFAULT_FILTERS } from './urlState.js'
...
describe('fetch wrappers', () => {
  ...
  it('setDecision POSTs a JSON body', async () => {
    ...
    expect(JSON.parse(opts.body)).toEqual({ hash: 'hash123', decision: 'keep' })
  })

  it('assignFace posts to the face endpoint and returns JSON', async () => {
    const res = await assignFace(5, { cluster_id: 2 })
    expect(fetch.mock.calls[0][0]).toBe('/api/faces/5/assign')
