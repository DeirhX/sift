...
import { thumbUrl, assignFace, deleteFace } from '../api'
import { fmt, aestheticScore, qualityColor } from '../format'
...
export default function PhotoCard(
  ...
  const [editFace, setEditFace] = useState<number | null>(null)   // index into item.faces
  ...
  const face = editFace != null ? item.faces[editFace] : undefined

  const runFace = async (fn: () => Promise<unknown>) => {
    if (busy) return
    setBusy(true)
    try {
      await fn()
  ...
  const onSelect = (e: ChangeEvent<HTMLSelectElement>) => {
    ...
    if (v === '__new__') {
      ...
      runFace(() => assignFace(face.id, { new_person: true, name: name ?? undefined }))
    } else {
