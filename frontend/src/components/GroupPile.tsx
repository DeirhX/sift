...
import { fmt, qualityColor } from '../format'
...
export default function GroupPile(
  { group, onOpen, focused = false }: { group: Group; onOpen: () => void; focused?: boolean },
) {
  ...
  const scores = items.map((i) => i.combined).filter((v): v is number => v != null)
