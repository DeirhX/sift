...
function buildTree(folders) {
  const root = { children: new Map(), direct: 0, path: null, sep: '/' }
  for (const { path, count } of folders) {
    ...
    for (const seg of segs) {
      ...
      if (!child) {
        child = { children: new Map(), direct: 0, path: acc, seg, sep }
...
function Node({ node, depth, selected, expanded, onToggle, onPick }) {
...
export default function FolderTree({ folders, filters, updateFilter }) {
